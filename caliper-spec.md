...
* serialize in-memory Caliper [Envelope](#envelope) objects as JSON as described in Section [5.2].  Serialized [Event](#event) and [Entity](#entity) *[describe](#describeDef)* data must be transmitted inside an [Envelope](#envelope) as described in Section [5.3](#jsonldPayload).
* transmit over HTTP serialized Caliper [Envelopes](#envelope) containing [Event](#event) and/or [Entity](#entity) *[describe](#describeDef)* data to a target [Endpoint](#endpoint) as described in Section [5.4](#httpRequest).
